...
void	test_comment(t_alloc *alloc)
{
	char	*input;

	input = str_dup(alloc, "echo a#REMOOVE");
	remove_comment(alloc, &input);
...
void	test_tokenize(t_alloc *alloc)
{
	char			*input;
	...
	input = str_dup(alloc, "cmd >> \">>\" << < 'cmd '");
	token_list = tokenize(alloc, &input);
	assert(token_list);
...
void	test_tokenize_pipes(t_alloc *alloc)
{
	char			*input;
	t_token_list	*token_list;
	t_token_list	*current;

	input = str_dup(alloc, "ls -la | grep test | wc -l");
	token_list = tokenize(alloc, &input);
	assert(token_list);
...
void	test_tokenize_quotes(t_alloc *alloc)
{
	char			*input;
	t_token_list	*token_list;

	// Test single quotes
	input = str_dup(alloc, "echo 'hello world'");
	token_list = tokenize(alloc, &input);
	assert(token_list);
	...
	input = str_dup(alloc, "echo \"hello $USER\"");
	token_list = tokenize(alloc, &input);
	assert(token_list);
	assert(token_list->next && token_list->next->content->type == E_WORD);
}

void	test_tokenize_edge_cases(t_alloc *alloc)
{
	char			*input;
	t_token_list	*token_list;
	...
	input = str_dup(alloc, "cmd > file >> append < input");
	token_list = tokenize(alloc, &input);
	assert(token_list);
	assert(token_list->content->type == E_WORD);
}

void	test_tilde_expanding(t_alloc *alloc)
{
	char			*input;
	t_token_list	*token_list;

	// Test basic tilde expansion (~)
	input = str_dup(alloc, "echo ~");
	token_list = tokenize(alloc, &input);
	...
	input = str_dup(alloc, "echo hello");
	token_list = tokenize(alloc, &input);
	tilde_expanding(alloc, token_list);
	assert(token_list->next && token_list->next->content->type == E_WORD);
...
void	test_parsing(void)
{
	...
	test_comment(alloc_test);
	printf("✓ Comment tests passed\n");
	
	test_tokenize(alloc_test);
	printf("✓ Basic tokenize tests passed\n");
	
	test_tokenize_pipes(alloc_test);
	printf("✓ Pipe tokenize tests passed\n");
	
	test_tokenize_quotes(alloc_test);
	printf("✓ Quote tokenize tests passed\n");
	
	test_tokenize_edge_cases(alloc_test);
	printf("✓ Edge case tests passed\n");
	
	test_tilde_expanding(alloc_test);
	printf("✓ Tilde expanding tests passed\n");
	printf("All parsing tests passed!\n");
	free_allocator(&alloc_test);
}
